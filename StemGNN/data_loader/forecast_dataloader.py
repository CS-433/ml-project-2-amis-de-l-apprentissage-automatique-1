...
class ForecastDataset(torch_data.Dataset):
    def __init__(self, df, target_df, window_size, horizon, normalize_method=None, norm_statistic=None, interval=1):
    ...
    def __getitem__(self, index):
        ...
        target_data = self.target_df[hi]  # Get the label for this sample
        x = torch.from_numpy(train_data).type(torch.float)
        y = torch.tensor(target_data).type(torch.long)  # Assuming the labels are categorical
        return x, y
