...
class Model(nn.Module):
    def __init__(self, units, stack_cnt, time_step, multi_layer, horizon=1, dropout_rate=0.5, leaky_rate=0.2,
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.time_step * 40, 64),  # flattened input to (batch_size, window size * 40) 
            nn.LeakyReLU(),
            nn.Linear(64, 3),  # Output 3 classes
            nn.Softmax(dim=-1)  # Softmax activation for classification probabilities
        )
    ...
    def forward(self, x):
        ...
        forecast = forecast.view(forecast.size(0), -1)  # flattening it to (batch_size, 40 * window size)
